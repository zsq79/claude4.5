...
var kiroAgentProfiles = []agentProfile{
	...
	{"", "", "spec", "en-US,en;q=0.9", ""},
	{"", "", "spec", "en-US,en;q=0.8,zh-CN;q=0.6", ""},
	{"", "", "preview", "en-US,en;q=0.85", ""},
	{"", "", "spec", "en-US,en;q=0.7,ja-JP;q=0.3", ""},
}
...
// 基于抓包数据：
// user-agent: aws-sdk-js/1.0.0 ua/2.1 os/win32#10.0.26200 lang/js md/nodejs#22.19.0 api/codewhispererruntime#1.0.0 m/E KiroIDE-0.4.0-{hash}
// x-amz-user-agent: aws-sdk-js/1.0.0 KiroIDE-0.4.0-{hash}
func (m *HeaderManager) kiroProfile(tokenIdentifier string) agentProfile {
	...
	// 使用哈希的不同部分来决定不同的属性，确保一致性
	versionChoice := int(seedHash[0]) % 100  // 使用第一个字节决定版本
	
	// 生成 Kiro 版本号（最新版本是 v0.4.0）
	// 基于稳定的哈希值，模拟真实的版本分布
	var kiroVersion string
	if versionChoice < 15 {
		// 15% 概率：旧版本 0.3.x（模拟未更新的用户）
		kiroPatch := 5 + (int(seedHash[1]) % 5)  // 0.3.5 - 0.3.9
		kiroVersion = fmt.Sprintf("0.3.%d", kiroPatch)
	} else {
		// 85% 概率：最新版本 0.4.0
		kiroVersion = "0.4.0"
	}
	...
	// 这个哈希每次会话都不同，但其他属性保持稳定
	kiroHash := generateKiroHash()
	kiroSignature := fmt.Sprintf("KiroIDE-%s-%s", kiroVersion, kiroHash)
	
	// 基于稳定种子生成操作系统信息
	osInfo := stableKiroOS(seedHash)
	
	// 基于稳定种子生成 Node.js 版本（18-22）
	nodeJSMajor := 18 + (int(seedHash[2]) % 5)        // 18-22
	nodeJSMinor := int(seedHash[3]) % 21              // 0-20
	...
	// agent mode 也基于稳定种子
	agentModeChoice := agentModes[int(seedHash[7])%len(agentModes)]
	...
	// x-amz-user-agent: 简化格式
	xAmzUA := fmt.Sprintf("aws-sdk-js/1.0.0 %s", kiroSignature)
	
	// user-agent: 完整格式
	userAgent := fmt.Sprintf("aws-sdk-js/1.0.0 ua/%d.%d %s lang/js md/nodejs#%d.%d.%d api/codewhispererruntime#1.0.0 m/%s %s",
		uaMajor, uaMinor,
		osInfo,
...
// generateKiroHash 生成 64 位十六进制哈希，模拟 KiroIDE 的签名
...
func applyLegacyHeaders(req *http.Request, isStream bool) {
	...
	req.Header.Set("x-amz-user-agent", "aws-sdk-js/1.0.18 KiroIDE-legacy")
	req.Header.Set("User-Agent", "aws-sdk-js/1.0.18 ua/legacy")
